...
Partial Class DashBoard_frmOnlineKPIL0
...
#Region "Custom Methods"
    ...
    Private Sub BindNSNDGSignature(ByVal wfid As Integer, ByVal wpid As String)
        ...
        If New DocController().IsParentAlreadyUploaded(Convert.ToInt32(hdnsiteid.Value), Integer.Parse(hdnversion.Value), Integer.Parse(hdndocid.Value)) = True Then
            If New CODInjectionController().IsAvalaibleDocInjection(Integer.Parse(hdndocid.Value), Request.QueryString("wpid")) = False Then
                hdnKeyVal.Value = 0
                ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScript", "Showmain('nyreg', " & hdndocid.Value & ");", True)
            End If
        End If
    ...
    Private Sub Binddata(ByVal wpid As String)
        ...
        Select Case i
            ...
            Case 2
                hdnKeyVal.Value = 0
                ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScript", "Showmain('Int', " & hdndocid.Value & ");", True)
            Case 3
                Dochecking()
            Case 4
                hdnKeyVal.Value = 0
                ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScriptI", "Showmain('IntD', " & hdndocid.Value & ");", True)
            Case 3
        End Select
            ...
            If objBO.verifypermission(hdndocid.Value, roleid, grp) <> 0 Then
                Select Case objBO.DocUploadverify(hdnsiteid.Value, hdndocid.Value, hdnversion.Value).ToString
                    ...
                    Case 4 'means document already processed for sencod stage cannot upload
                        hdnKeyVal.Value = 0
                        ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScripta", "Showmain('2sta', " & hdndocid.Value & ");", True)
                        'makevisible()
                        Exit Sub
            ...
            Else
                If Session("Role_Id") = 1 Then ''if role is sysadmin then no need to verify permission everthing permitted.
                    Select Case objBO.DocUploadverify(hdnsiteid.Value, hdndocid.Value, hdnversion.Value).ToString
                        ...
                        Case 4 'means document already processed for sencod stage cannot upload
                            hdnKeyVal.Value = 0
                            ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScriptb", "Showmain('2sta', " & hdndocid.Value & ");", True)
                            Exit Sub
                            'Response.Redirect("frmSiteDocUploadTree.aspx")
                    End Select
                Else
                    ...
                    hdnKeyVal.Value = 0
                    ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScriptc", "Showmain('nop', " & hdndocid.Value & ");", True)
                End If
            End If
    ...
    Private Sub uploaddocument(ByVal vers As Integer, ByVal keyval As Integer)
        ...
        Response.Redirect("frmKPIL0RC.aspx")        
    End Sub
...
#End Region
    ...
    Private Sub SendGenerateMail()
        ...
        If dtuseremail.Rows.Count > 0 Then
            ...
            sbBody.Append("<a href='http://www.telkomsel.nsnebast.com'>Click here</a>" & " to login to E-KPIL0 <br/>")
            sbBody.Append("Powered By EBAST" & "<br/>")
            sbBody.Append("<img src='http://www.telkomsel.nsnebast.com/images/nsn-logo.gif' alt='companylogo' />")
