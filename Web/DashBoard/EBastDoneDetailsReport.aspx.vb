...
Imports System.Data
Imports System.IO
Partial Class EBastDoneDetailsReport
    ...
    Sub bindData()
        ...
        If dtra.Rows.Count = 0 Then
            strsql = "Exec [uspEBASTDoneDetailsReport] " + Request.QueryString("id") & "," & ConfigurationManager.AppSettings("BAUTID") & "," & ConfigurationManager.AppSettings("BASTID")
        Else
            area = dtra.Rows(0).Item(0).ToString
            region = dtra.Rows(0).Item(1).ToString
            If region <> 0 Then
                'regional user
                strsql = "Exec [uspEBASTDoneDetailsNew2Report] " + Request.QueryString("id") & "," & ConfigurationManager.AppSettings("BAUTID") & "," & ConfigurationManager.AppSettings("BASTID") & "," & Session("User_Id")
            ElseIf area <> 0 Then
                'area user
                strsql = "Exec [uspEBASTDoneDetailsNew3Report] " + Request.QueryString("id") & "," & ConfigurationManager.AppSettings("BAUTID") & "," & ConfigurationManager.AppSettings("BASTID") & "," & Session("User_Id")
            Else
                'national user
                strsql = "Exec [uspEBASTDoneDetailsReport] " + Request.QueryString("id") & "," & ConfigurationManager.AppSettings("BAUTID") & "," & ConfigurationManager.AppSettings("BASTID")
            End If
