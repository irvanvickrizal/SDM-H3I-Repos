...
Partial Class verifySignature1
    ...
    Protected Sub Page_LoadComplete(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.LoadComplete
        ...
        If Request.QueryString("rdx") = "0" Then
            ...
            If dt1.Rows.Count() > 0 Then
                ...
                url = "window.location='verifySignature2.aspx?rdx=" & userid & "&name=" & username & "&docpath=" & docpath & "&records=" & records.ToString
        ...
        If Request.QueryString("rdx") <> "" And Request.QueryString("rdx") <> "-1" Then
            ...
            If dt1.Rows.Count > 0 Then
                ...
                username = dt1.Rows(0).Item("username").ToString.Replace(" ", "^").Replace("\", "|")
                docpath = dt1.Rows(0).Item("docpath").ToString.Replace(" ", "^").Replace("\", "|")
                url = "window.location='verifySignature2.aspx?rdx=" & userid & "&name=" & username & "&docpath=" & docpath & "&records=" & Request.QueryString("records")
                Response.Write("<script language='JavaScript'>" & url & "';</script>")
            Else
                url = "window.location='verifySignature2.aspx?rdx=-1"
                Response.Write("<script language='JavaScript'>" & url & "';</script>")
            End If
        End If
    End Sub
