...
Partial Class PO_frmPOList
    ...
    Sub bindData()
        If Request.QueryString("isnull") = "1" Then
            dt = objbo.uspPODetailsList("", "", "isnull", 0)
            grdPOrawdata.DataSource = dt
            'grdPOrawdata.PageSize = Session("Page_size")
            grdPOrawdata.DataBind()
        Else
            dt = objbo.uspPODetailsList(ddlPO.SelectedValue, ddlSelect.SelectedValue, txtSearch.Text.Replace("'", "''"), 0)
            grdPOrawdata.DataSource = dt
            'grdPOrawdata.PageSize = Session("Page_size")
            grdPOrawdata.DataBind()
        End If
