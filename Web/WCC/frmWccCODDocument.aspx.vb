...
Imports Common
Imports System.Data
Partial Class WCC_frmWccCODDocument
    ...
    End Sub
    Sub BindTree()
        dt = objutil.ExeQueryDT("Select Doc_Id,DocName from WccCodDoc where Parent_id=0 order by SerialNo", "WccCODDoc")
    ...
    Sub fillchild(ByVal tn As TreeNode)
        dt = objutil.ExeQueryDT("Select Doc_Id,DocName,(select count(*) from WccCodDoc as c where c.parent_id=parent_Id) as cc from WccCodDoc where parent_Id = " & tn.Value & " and rstatus=2 group by parent_id,Doc_Id,DocName", "WccCODDoc")
        For Each drow As DataRow In dt.Rows
            Dim node As New TreeNode
            Dim str As New StringBuilder
            str.Append(drow.Item("DocName").ToString)
    ...
    End Sub
    Sub binddata()
        dt = objbo.uspWccCODDocumentList(, ddlSelect.SelectedValue, txtSearch.Text, ddlSectionSrc.SelectedValue, hdnSort.Value, 0)
    ...
    Sub sec_binddata()
        objBL.fillDDL(ddlScopeSrc, "WccCODScope", True, Constants._DDL_Default_Select)
        objBL.fillDDL(ddlScope, "WccCODScope", True, Constants._DDL_Default_Select)
        objBL.fillDDL(ddlSectionSrc, "WCCCodDocNew", True, Constants._DDL_Default_Select)
    End Sub

    Sub BindDescription()
        dt = objbo.uspWccCODDocumentList(Request.QueryString("id"))
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If TVDoc.Visible = True Then
            ...
            If Request.QueryString("id") Is Nothing Then
                i = objbo.uspWccCODDocumentIU(objdo)
                If i > 2 Then
                    ADDXY(i)
                    BOcommon.result(1, True, "frmWccCODDocument.aspx", "Document Name", Constants._INSERT)
                ElseIf i < 0 Then
                    BOcommon.result(2, True, "frmWccCODDocument.aspx", "Document Name", Constants._INSERT)
                Else
                    BOcommon.result(1, True, "frmWccCODDocument.aspx", "Document Name", Constants._INSERT)
                End If
            Else
                ...
                ADDXY(objdo.DOC_ID)
                BOcommon.result(objbo.uspWccCODDocumentIU(objdo), True, "frmWccCODDocument.aspx", "Document Name", Constants._UPDATE)
            End If
        End If
    End Sub
    ...
    Protected Sub btnDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        BOcommon.result(objbo.uspDelete("WccCODDoc", "DOC_ID", Request.QueryString("id")), True, "frmWccCODDocument.aspx", " ", Constants._DELETE)
    End Sub

    Protected Sub btnCanel_ServerClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCanel.ServerClick
        Response.Redirect("frmWccCODDocument.aspx")
    End Sub
