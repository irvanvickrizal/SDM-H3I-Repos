...
Partial Class WCC_frmWccWorkFlowDocSetup
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            ddlWF.SelectedValue = 7 'Request.QueryString("id")
            ddlWF.Enabled = False
    ...
    End Sub
    Sub checkTree()
        dt = objBo.uspWccWFDocList(ddlWF.SelectedValue)
    ...
    End Function
    Sub FillSection()
        dt = objutil.ExeQueryDT("Select C.Doc_Id,DocName from WccCodDoc C Left Outer join CodWFDoc W on W.Doc_Id=C.Doc_Id where Parent_id=0 order by SerialNo", "CODDoc")
        If dt.Rows.Count > 0 Then
    ...
    Sub FillChild(ByVal tn As TreeNode)
        If tn.Value <> 0 Then
            dt = objBo.uspWccBOTaskSubDoc(tn.Value, Constants._WFReq)
            For Each drow As DataRow In dt.Rows
                Dim node As New TreeNode
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        For Each pchild As TreeNode In TreeView1.CheckedNodes
            ...
            If pchild.Value > 0 Then str = str & IIf(str = "", "", ",") & "'" & pchild.Value & "'"

        Next
        Dim strDocName As String = ""
        If str <> "" Then
            dt = objBo.uspWccWFDocCheck(ddlWF.SelectedValue, str)
            If dt.Rows.Count > 0 Then
        ...
        Else
            Dim i As Integer = -1
            If str <> "" Then
                i = objBo.uspWccBoWTDocInsert(ddlWF.SelectedValue, Constants.STATUS_ACTIVE, Session("UserName"), str)
                If i = 0 Then
                ...
                ElseIf i = 1 Then
                    ...
                    Response.Redirect("frmWccWorkFlowDocSetup.aspx")
                End If
            Else
                If objutil.ExeQueryScalar("Select Count(*) from wccSiteDoc where WF_id=" & ddlWF.SelectedValue) = 0 Then
                    i = objBo.uspWccBoWTDocInsert(ddlWF.SelectedValue, Constants.STATUS_ACTIVE, Session("UserName"), str)
                    If i = 0 Then
                        ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScript", "alert('Error while doing the transaction.');", True)
                    ElseIf i = 1 Then
