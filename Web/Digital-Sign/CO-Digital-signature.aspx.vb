...
Partial Class Digital_Sign_CO_Digital_signature
        ...
        If j > 0 Then
            Try
                ...
                If (strResult = Constants._DigitalSign_Result) Then
                    Dim i As Integer = -1
                    If co_controller.CODocApproved(Convert.ToInt32(Request.QueryString("id")), Convert.ToInt32(hdncoid.Value), Integer.Parse(hdnwfid.Value), Integer.Parse(hdnTaskid.Value), Integer.Parse(hdnRoleId.Value), CommonSite.UserId) = True Then
                        i = 1
                    Else
                        i = 0
                    End If
                    ...
                    ElseIf i = 1 Then
                        If Integer.Parse(hdnTaskid.Value) > 0 Then
                            SendMailNextUser(co_controller.GetUserTaskPendingEmail(Convert.ToInt32(Request.QueryString("id")), "approval"), Convert.ToInt32(hdncoid.Value), String.Empty)
                        End If
                    End If
    ...
    Protected Sub grddocuments_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles grddocuments.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim LblViewLog As Label = CType(e.Row.FindControl("LblViewLog"), Label)
            Dim rdbstatus As RadioButtonList = CType(e.Row.FindControl("rdbstatus"), RadioButtonList)
            If LblDocType.Text.ToLower().Equals("crfinal") Then
                rdbstatus.Visible = False
                LblViewLog.Visible = True
                url = "../CR/frmViewCRFinal_Popup.aspx?wpid=" & Request.QueryString("wpid")
            Else
...
#Region "Custom Methods"
    Private Sub ConfigureHiddenField(ByVal sno As Int32, ByVal wpid As String)
        ...
        If Not info Is Nothing Then
            ...
            MvCorePanel.SetActiveView(VwMainPanel)
        Else
            MvCorePanel.SetActiveView(VwMessagePanel)
        End If
    End Sub
    ...
    Private Sub ConfigurePanel()
        MvApprovalButtonPanel.SetActiveView(vwDefaultPanel)
        MvPanelSign.SetActiveView(defaultSign)
        If hdnTaskType.Value.Equals("app") Then
    ...
    Private Sub BindAdditionalDocument()
        If hdnTaskType.Value.Equals("app") Then
            grddocuments.DataSource = co_controller.GetCOAttachDocument(Integer.Parse(ConfigurationManager.AppSettings("codocid")), Request.QueryString("wpid"), Convert.ToInt32(hdnsiteid.Value), Integer.Parse(hdnVersion.Value))
            grddocuments.DataBind()
        Else
            grddocuments2.DataSource = co_controller.GetCOAttachDocument(Integer.Parse(ConfigurationManager.AppSettings("codocid")), Request.QueryString("wpid"), Convert.ToInt32(hdnsiteid.Value), Integer.Parse(hdnVersion.Value))
    ...
    Private Sub SubmitSignReject(ByVal remarks As String, ByVal catReason As String)
        ...
        If (wpid IsNot Nothing) Then
            ...
            Dim i As Integer = -1
            If co_controller.CODocRejected(Convert.ToInt32(Request.QueryString("id")), Convert.ToInt32(hdncoid.Value), Integer.Parse(hdnwfid.Value), Integer.Parse(hdnTaskid.Value), Integer.Parse(hdnRoleId.Value), CommonSite.UserId, remarks, catReason) = True Then
                i = 1
            Else
                i = 0
            End If
            ...
            ElseIf i = 1 Then
                Try
                    SendMailNextUser(co_controller.GetUserTaskPendingEmail(Convert.ToInt32(Request.QueryString("id")), "rejection"), Convert.ToInt32(hdncoid.Value), remarks)
    ...
    Private Sub SendMailNextUser(ByVal emails As List(Of EmailInfo), ByVal coid As Int32, ByVal remarks As String)
        ...
        Else
            company = "Telkomsel"
        End If
        Dim codetailinfo As COInfo = GetCODetail(coid)
        Dim emailsubject As String = String.Empty
        If statusType.Equals("Approved") Then
            sb.Append("Following detail of CO Document already approved by " & appvduser & " " & company & "<br/>")
            emailsubject = "CO Approved"
        ElseIf statusType.Equals("Rejected") Then
            sb.Append("Following detail of CO Document was rejected by " & appvduser & " " & company & "<br/>")
            emailsubject = "CO Rejected"
        Else
            sb.Append("Following detail CO Document is waiting your approval/review <br/>")
            emailsubject = "Document CO Waiting"
        End If
        sb.Append("Docname : " & "CO Online" & "<br/>")
        sb.Append("SiteNo :" & codetailinfo.SiteNo & "<br/>")
        sb.Append("SiteName :" & codetailinfo.SiteName & "<br/>")
        sb.Append("Scope :" & codetailinfo.Scope & "<br/>")
        sb.Append("HOT/HOG :" & codetailinfo.PONO & "<br/>")
        sb.Append("PO Name :" & codetailinfo.EOName & "<br/>")
        sb.Append("Workpackageid :" & codetailinfo.PackageId & "<br/>")
        If statusType.Equals("Rejected") Then
            sb.Append("Reason of Rejection : " & remarks & "<br/><br/>")
        End If

        sb.Append("<a href='http://www.nsndemo.nsnebast.com:1300'>Click here</a>" & " to go to e-BAST<br/>")
        sb.Append("Powered By EBAST" & "<br/>")
        sb.Append("<img src='http://www.nsndemo.nsnebast.com:1300/images/nsn-logo.gif' alt='companylogo' />")
