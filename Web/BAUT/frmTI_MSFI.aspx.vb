...
#End Region
    ...
    Protected Sub GvMasterList_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs) Handles GvMasterList.RowCommand
        If e.CommandName.Equals("deletesitedoc") Then
            If msficontrol.MSFIDetail_D(Convert.ToInt32(hdnMSFIID.Value), Integer.Parse(e.CommandArgument.ToString())) Then
                BindMasterlist(Convert.ToInt32(hdnMSFIID.Value))
            End If
        End If
    End Sub

    Protected Sub gvMSFIList_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles gvMSFIList.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim parentid As Integer = Integer.Parse(DataBinder.Eval(e.Row.DataItem, "parent_id"))
            Dim pnlCategory As Panel = CType(e.Row.FindControl("pnlCategory"), Panel)
            Dim txtRemarks As TextBox = CType(e.Row.FindControl("txtRemarks"), TextBox)
            Dim sn As String = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "sn"))
            Dim docname As String = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "docname"))
            Dim ltrSN As Literal = CType(e.Row.FindControl("ltrSN"), Literal)
            Dim ltrDocName As Literal = CType(e.Row.FindControl("ltrDocName"), Literal)

            If pnlCategory IsNot Nothing Then
                If parentid = 0 Then
            ...
            If ltrSN IsNot Nothing And ltrDocName IsNot Nothing Then
                If parentid > 0 Then
                    If (Not String.IsNullOrEmpty(sn)) Then
                        ltrSN.Text = "&nbsp;&nbsp;" & sn & "."                        
                    End If

                    ltrDocName.Text = docname
                Else
                    If Not String.IsNullOrEmpty(sn) Then
                        ltrSN.Text = "<b>" & sn & ".</b>"
                    End If

                    ltrDocName.Text = "<b>" & docname & "</b>"
                End If
            End If
        End If
    End Sub

    Protected Sub GvMasterList_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles GvMasterList.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            Dim parentid As Integer = Integer.Parse(DataBinder.Eval(e.Row.DataItem, "parent_id"))
            Dim isChecked As Integer = Integer.Parse(DataBinder.Eval(e.Row.DataItem, "isChecked"))
            Dim sn As String = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "sn"))
            Dim docname As String = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "docname"))
            ...
            Dim ltrSN As Literal = CType(e.Row.FindControl("ltrSN"), Literal)
            Dim ltrDocName As Literal = CType(e.Row.FindControl("ltrDocName"), Literal)
            If chkChecked IsNot Nothing And imgDelete IsNot Nothing Then
                If parentid > 0 Then
...
#Region "custom methods"
    Private Sub BindData(ByVal wpid As String)
        hdnMSFIID.Value = msficontrol.MSFI_GetID(wpid, CommonSite.UserId, String.Empty)
        'Dim info As MSFIInfo = msficontrol.ODmsfi_LD(wpid)
        ...
        CheckMasterlist(Convert.ToInt32(hdnMSFIID.Value))
        BindDocDetail()
    ...
    Private Sub CheckMasterlist(ByVal msfiid As Int32)

        Dim dtDefinedList As DataTable = msficontrol.MSFIDetail_GetList(msfiid)
        If dtDefinedList.Rows.Count > 0 Then
            BindDefinedList(dtDefinedList)
        Else
            BindMasterList(msfiid)
        End If
    End Sub

    Private Sub MSFIChecklistCheck()
        Dim j As Integer
        Dim checkcount As Integer = 0
        Dim isSucceed As Boolean = True
        For j = 0 To GvMasterList.Rows.Count - 1
            Dim chk As New CheckBox
            Dim lblCheckedCount As Label = GvMasterList.Rows(j).Cells(0).FindControl("lblCheckedCount")
            chk = GvMasterList.Rows(j).Cells(0).FindControl("chkChecked")
            If chk.Checked = True Then
                Dim lblListID As Label = GvMasterList.Rows(j).Cells(0).FindControl("lblListID")                
                If Not String.IsNullOrEmpty(lblListID.Text) Then
                    If (msficontrol.MSFIDetail_I(Integer.Parse(lblListID.Text), Convert.ToInt32(hdnMSFIID.Value), CommonSite.UserId)) Then
                        checkcount += 1
                    End If
                End If
            End If

            If Integer.Parse(lblCheckedCount.Text) > 0 Then
                checkcount += 1
            End If
        Next
        ...
        If checkcount > 0 Then
            ...
            End If
        Else
            ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScript", "alert('At least 1 checklist is thicked!');", True)
        End If

        
    End Sub

    Private Sub MSFIDetailChecklistCheck()
        Dim j As Integer
        Dim checkcount As Integer = 0
        Dim isSucceed As Boolean = True
        For j = 0 To gvMSFIList.Rows.Count - 1
            Dim chkCopy As New CheckBox
            Dim chkOriginal As New CheckBox
            Dim chkNA As New CheckBox
            Dim txtRemarks As New TextBox
            Dim lblMSFIDetID As Label = gvMSFIList.Rows(j).Cells(0).FindControl("lblMSFIDetID")
            Dim lblParentID As Label = gvMSFIList.Rows(j).Cells(0).FindControl("lblParentID")            
            chkCopy = gvMSFIList.Rows(j).Cells(0).FindControl("chkCopy")
            chkOriginal = gvMSFIList.Rows(j).Cells(0).FindControl("chkOriginal")
            chkNA = gvMSFIList.Rows(j).Cells(0).FindControl("chkNA")
            txtRemarks = gvMSFIList.Rows(j).Cells(0).FindControl("txtRemarks")
            If lblParentID IsNot Nothing And lblMSFIDetID IsNot Nothing Then
                If Integer.Parse(lblParentID.Text) > 0 Then
                    If Not String.IsNullOrEmpty(txtRemarks.Text) Then
                        If msficontrol.MSFIDetail_Category_U(Convert.ToInt32(lblMSFIDetID.Text), IIf(chkCopy.Checked = True, True, False), IIf(chkOriginal.Checked = True, True, False), IIf(chkNA.Checked = True, True, False), txtRemarks.Text, CommonSite.UserId) = False Then
                            isSucceed = False
                        End If
                    Else
                        If msficontrol.MSFIDetail_Category_U(Convert.ToInt32(lblMSFIDetID.Text), IIf(chkCopy.Checked = True, True, False), IIf(chkOriginal.Checked = True, True, False), IIf(chkNA.Checked = True, True, False), String.Empty, CommonSite.UserId) = False Then
                            isSucceed = False
                        End If
                    End If
        ...
        If isSucceed Then
            Response.Redirect("frmMSFI_Final.aspx?mid=" & hdnMSFIID.Value & "&wpid=" & GetWPID() & "&siteid=" & hdnsiteid.Value & "&vers=" & hdnVersion.Value & "&swid=" & GetSWID())
        Else
            ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScript", "alert('Error while saving the data, please try again!');", True)
        End If

    End Sub

    Private Sub BindMasterList(ByVal msfiid As Int32)
        strsql = "Exec Masterlist_getALL " & msfiid
        dt = objUtil.ExeQueryDT(strsql, "Masterlist_getALL")
        mvPanelList.SetActiveView(vwMasterList)
    ...
    Private Sub BindDefinedList(ByVal dtResult As DataTable)
        mvPanelList.SetActiveView(vwDefinedList)
        gvMSFIList.DataSource = dtResult
        gvMSFIList.DataBind()
    ...
    Private Sub uploaddocument(ByVal vers As Integer, ByVal keyval As Integer)
        ...
        Else
            Response.Redirect("../HCPT_Dashboard/frmmsfiReadyCreation.aspx")
        End If
