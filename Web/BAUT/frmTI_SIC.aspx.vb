...
Partial Class BAUT_frmTI_SIC
    ...
    Sub tableHeader(ByRef submit As Boolean)
        ...
        Dim strsql As String = "select * from odsicvalue as sicvalue inner join odsic as sic on sicvalue.sno = sic.sno where swid=" & Request.QueryString("id")
    ...
    Sub uploaddocument(ByVal vers As Integer, ByVal keyval As Integer)
        ...
        FileNameOnly = "-" & Constants_NSN._Doc_SIC & "-"
    ...
    Function CreatePDFFile(ByVal StrPath As String) As String
        ...
        Dim filenameorg As String
        Dim ReFileName As String
        filenameorg = hdnSiteno.Value & "-SIC-" & Format(CDate(DateTime.Now), "ddMMyyyyHHss")
        ReFileName = filenameorg & ".htm"
    ...
    Function saveData() As String
        ...
        If Not Request.QueryString("rw") = "" Then
            rw = Request.QueryString("rw")
        End If
        strsql = "delete sicvalue from odsicvalue sicvalue inner join odsic sic on sicvalue.sno = sic.sno and sic.swid=" & Request.QueryString("id")
        ...
        While 1
            ...
            Next
            items = items.Replace("@@", ",")
            strsql = "Exec odsicinsert " & Request.QueryString("id") & " ,'" & lblPONO.Text & "','" & hdnsiteid.Value & "','" & hdnversion.Value & "'," & _
            ...
            "'" & txtSACRNo.Text & "'," & IIf(txtSACRD.Value.ToString <> "", "'" & txtSACRD.Value & "'", "null") & "," & "null" & "," & _
            "" & "null" & ",'" & txtOAir.Text & "'," & IIf(txtOAirD.Value.ToString <> "", "'" & txtOAirD.Value & "'", "null") & "," & _
            items & _
            "'" & Session("User_Name") & "'"
    ...
    Protected Sub btnAddRow_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddRow.Click
        ScriptManager.RegisterStartupScript(Page, GetType(UpdatePanel), "MyScriptdd", "getControlPosition();", True)
        Session("txtSicRefNo_SIC") = txtSicRefNo.Text
        Session("txtDate_SIC") = txtDate.Value
        Session("txtRef_SIC") = txtRef.Text
        Session("txtDated_SIC") = txtDated.Value
        Session("txtCOR_SIC") = txtCOR.Text
        Session("txtCORDated_SIC") = txtCORDated.Value
        Session("txtSACRNo_SIC") = txtSACRNo.Text
        Session("txtSACRD_SIC") = txtSACRD.Value
        'Session("txtICRNo") = txtICRNo.Text
        'Session("txtICRD") = txtICRD.Value
        Session("txtOAir_SIC") = txtOAir.Text
        Session("txtOAirD_SIC") = txtOAirD.Value
